...
public class FrontEnd {
    ...
    public static Prodotto inserimentoSmartphone(Scanner sc) throws Exception {
        ...
        System.out.println("Inserire il prezzo dello smartphone");
        double prezzo = Double.parseDouble(sc.nextLine());
        if(prezzo < 0){
            throw Eccezioni.PREZZONONVALIDO;
        }
        System.out.println("Inserire la memoria dello smartphone");
        int memoria = Integer.parseInt(sc.nextLine());
        if(memoria < 0){
            throw Eccezioni.MEMORIANONVALIDA;
        }
        System.out.println("Inserire la quantità dello smartphone");
        int quantita = Integer.parseInt(sc.nextLine());
        if(quantita < 0){
            throw Eccezioni.QUANTITANONVALIDA;
        }
        int nuovoCodice = ++codice;
        if(esisteCodice(nuovoCodice)) {
            throw Eccezioni.CODICEESISTENTE;
        }
        Smartphone s = new Smartphone(nuovoCodice, marca, prezzo, quantita, modello, memoria);
        prodotti.add(s.clone());
        return s;
    }
    ...
    public static boolean rimuoviProdotto(int codice) {
        return prodotti.removeIf(p->p.getCodice() == codice);
    }
